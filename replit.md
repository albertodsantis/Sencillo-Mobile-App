...
- **Data Persistence**: Repository pattern (`lib/repositories/`) with AsyncStorage backend. Each domain entity has its own repository (TransactionRepository, ProfileRepository, RatesRepository, PnlRepository, BudgetRepository, SavingsRepository). The old `lib/data/storage.ts` is preserved but no longer imported — all data access goes through repositories. Storage keys are prefixed with `@sencillo/`. IDs are generated with `Crypto.randomUUID()`. This architecture is designed for easy migration to Supabase or other backends
